...
export function parseScreenplay(text: string, episodeCount?: number): {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (line.startsWith("△")) {
      ...
      // 判断景别
      let shotType = "MS";
      if (/全景|远景|俯瞰|整间|整个|四周/.test(action)) shotType = "WS";
      else if (/特写|近距|手指|眼睛|瞳孔|指尖/.test(action)) shotType = "ECU";
      else if (/面部|表情|脸|眼神|嘴角/.test(action)) shotType = "CU";
      else if (/特效|瞬间|光芒|魔法|修复|消失|化成/.test(action)) shotType = "VFX";
    ...
    if (dialogueMatch) {
      ...
      shots.push({
        ...
        shot_type: isOS ? "MS" : "MCU",
        action: charAction ? `${character}${charAction}` : `${character}说话`,
...
async function step2b_shotPrompts(
  ...
  for (const sc of ep.scenes) {
    for (let i = 0; i < (sc.shots || []).length; i++) {
      ...
      shotLines.push(
        `${key} [镜头${globalShotIdx}]: ${shot.shot_type} | 主体:${shot.subject} | 动作:${shot.action} | 运镜:${shot.camera_movement} | 光学:${shot.optics || "50mm"} | 构图:${shot.composition || "center"} | 场景:${sc.scene_ref || sc.scene_id} | 情绪:${emotionHint} | 对话:${dialogueHint}`,
      );
    }
  }
  ...
  const totalShots = globalShotIdx;
  ...
  const systemPrompt = `你是AI分镜提示词专家。为给定的第${ep.episode_id}集的 ${totalShots} 个镜头按叙事顺序生成英文 visual_prompt。只输出JSON，不要代码块。${prevContext}

关键要求：
1. 每个 visual_prompt 约80-120个英文单词
2. 镜头之间必须体现**叙事递进**：动作、情绪、环境细节必须随剧情推进而变化，严禁重复
3. 相邻镜头之间要有**视觉衔接**：前一镜头的结尾状态 = 后一镜头的起始状态
4. 同一角色在不同镜头中的外貌描述必须完全一致
5. 每个镜头必须有明确的**角色动作**（不能只有站立/静止）
6. 本集的画面必须与前面集数的画面**完全不同**——不同的角色动作、不同的场景状态、不同的情绪表达

格式：[光学参数] + [景别构图] + [主体外貌+精确动作含微动作] + [环境背景细节] + [灯光色彩] + [摄影机运动] + ${styleKeywords}

输出格式（key 必须与输入完全一致）:
{"${exampleKey1}":"...","${exampleKey2}":"..."}`;

  const result = await wfAiChatStream(
    { model: await getChatModel(), messages: [{ role: "system", content: systemPrompt }, { role: "user", content: `第${ep.episode_id}集镜头列表（按叙事顺序）:\n${shotLines.join("\n")}` }], temperature: 0.7 },
    onProgress,
  );
